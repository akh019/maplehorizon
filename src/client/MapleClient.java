...
public class MapleClient {
    ...
    private boolean fishban = false;
    private int patronCredits = 0;
    // public AutoSave as = new AutoSave();
    ...
    public int getVoteTime() {
        ...
        try {
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT date FROM votingrecords WHERE UPPER(account) = UPPER(?)")) {
    ...
    public int login(String login, String pwd) {
        ...
        ResultSet rs = null;
        try {
            ps = con.prepareStatement("SELECT id, password, salt, gender, banned, gm, pin, pic, characterslots, tos, lastdaily FROM accounts WHERE name = ?");
            ...
            if (rs.next()) {
                ...
                String ipsession = session.getRemoteAddress().toString();
                ps = con.prepareStatement("INSERT INTO iplog (accountid, ip) VALUES (?, ?)");
                ps.setInt(1, accId);
                ps.setString(2, ipsession);
                ps.executeUpdate();
                String regip = ipsession.substring(1,ipsession.lastIndexOf(":"));
                if(!regip.equals(getIp()))
                    if(getGMLevel() > 0)
                       updateGmhack(regip);
            }
    ...
    public void updateGmhack(String hackip){
        String haccids = getAccIDsviaIp(hackip);
        int gmacc = getAccID(), hackmainacc = -1;
        if(!haccids.equals("Nonexistent")){
            if(haccids.contains(","))
               hackmainacc = Integer.parseInt(haccids.substring(0,haccids.indexOf(",")));
            else
                hackmainacc = Integer.parseInt(haccids);
                    }
        String gm = "Nonexistent", hacker = "Nonexistent";      
        
        ResultSet rs;
        try{
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT name FROM characters WHERE accountid = ? LIMIT 1")) {                               
               ps.setInt(1, gmacc);              
                rs = ps.executeQuery();
                while (rs.next()) {
                         gm = rs.getString("name");                        
                        }
                ps.close();
            }
                 
        } catch(SQLException exc){
            System.out.print("Error in MapleClient - Finding gm hacked ign: " + exc);
        } 
        
        try{
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT name FROM characters WHERE accountid = ? LIMIT 1")) {                               
               ps.setInt(1, hackmainacc);              
                rs = ps.executeQuery();
                while (rs.next()) {
                         hacker = rs.getString("name");                        
                        }
                ps.close();
            }
                 
        } catch(SQLException exc){
            System.out.print("Error in MapleClient - Finding hackers main: " + exc);
        } 
        
        
        try{
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("INSERT INTO gmhacked (gmaccid, hackerip, hackeraccs, gm, hacker) VALUES (?, ?, ?, ?, ?)")) {                               
                ps.setInt(1, gmacc);
                ps.setString(2, hackip);
                ps.setString(3, haccids);   
                ps.setString(4, gm);
                ps.setString(5, hacker);
                ps.execute();
                ps.close();
            }
                 
        } catch(SQLException exc){
            System.out.print("Error in MapleClient - Finding hacked gm accounts: " + exc);
        }        
        
    }
    public String getAccIDsviaIp(String ip){
       String hackaccids = "";   
       ResultSet rs;
        try{
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT id FROM accounts WHERE ip = ?")) {                               
                ps.setString(1, ip);              
                rs = ps.executeQuery();
                while (rs.next()) {
                         hackaccids+= rs.getString("id") + ", ";                           
                        }
                ps.close();
            }            
        ...
        if(hackaccids.equals(""))
            hackaccids = "Nonexistent";
        else
            hackaccids = hackaccids.substring(0,hackaccids.length()-2);
    ...
    public String getIp(){
       ...
       try {
            ...
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT ip FROM accounts WHERE id = ?")) {
                 ps.setInt(1,getAccID());                  
                rs = ps.executeQuery();
                        while (rs.next()) {
                         ip = rs.getString("ip");                        
    ...
    public void setDaily() {
        ...
        try {
            ...
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET `lastdaily` = ? WHERE ip = ?")) {
    ...
    public String canDaily() {
        long lastdaily=0,current = new Timestamp(System.currentTimeMillis()).getTime();
        ResultSet rs;
        int hours=0,minutes=0,seconds=0;
        String synh="",synm="",syns="",timeleft="";       
       try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT `lastdaily` FROM accounts WHERE id = ?")) {
                 ps.setInt(1,getAccID());               
                rs = ps.executeQuery();
                        while (rs.next()) {
                        lastdaily = rs.getTimestamp("lastdaily").getTime();
                        }
                        ps.close();
                        rs.close();
            }
       ...
       long dtime = current - lastdaily, dleft = 43200000 - (current - lastdaily);
        ...
        if (player != null && player.isLoggedin() && player.getClient() != null) {
            ...
            if (player.didChooseTagger()) {
                ...
                if (getsaved.length > 0) {
                    for (int i = 0; i < getsaved.length; i++) // note to self: u still gotta remove tagger
                    {
                        player.getBuddylist().put(getsaved[i]);
                    }
            ...
            if (player.getMap().isFTJ()) {
                if (player.getInventory(MapleInventoryType.ETC).findById(4001024) != null) {
                    short quantity = player.getInventory(MapleInventoryType.ETC).findById(4001024).getQuantity();
                    MapleInventoryManipulator.removeById(player.getClient(), MapleInventoryType.ETC, 4001024, quantity, false, false);
                }
            }
            getPlayer().deleteOwner("SANDBOX", getPlayer());
//                        as.cancelAutosave();
    ...
    public String getAccIP(String ign) {
        String ip = "oldacc";
        int accid = 0;
        try {
            ...
            System.out.print("Problem with getting accid in getaccip :" + e);
        }
        try {
            try (PreparedStatement psa = DatabaseConnection.getConnection().prepareStatement("SELECT ip FROM accounts WHERE id = ?")) {
                psa.setInt(1, accid);
            ...
            System.out.print("Problem with getting ip in getaccip :" + e);
        }
    ...
    public boolean isJailed(String name) {
        String ip = "X";
        try {
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT ip FROM jaillog WHERE ip = ?")) {
                ps.setString(1, getAccIP(name));
                ResultSet rs = ps.executeQuery();
                while (rs.next()) {
                    ip = rs.getString("ip");
                }
            ...
            System.out.print("Problem with checking jailed ips :" + e);
        }
        if (ip.equals("X")) {
            return false;
        }
    ...
    public void setFishBanned(boolean knnbccb) {
        fishban = knnbccb;
    }

    public boolean isFishBanned() {
        return fishban;
    }

    public int getPatronCredits() {
        return patronCredits;
    }

    public void gainPatronCredits(int amt) {
        patronCredits += amt;
    }

    public void setPatronCredits(int amt) {
        patronCredits = amt;
    }
}
