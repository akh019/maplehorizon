...
public final class ChangeMapHandler extends AbstractMaplePacketHandler {
	...
	public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		MapleCharacter chr = c.getPlayer();
                if(chr.getMap().isFTJ()){
                    if(chr.getInventory(MapleInventoryType.ETC).findById(4001024) != null){
                        short quantity = chr.getInventory(MapleInventoryType.ETC).findById(4001024).getQuantity();
                        MapleInventoryManipulator.removeById(chr.getClient(), MapleInventoryType.ETC, 4001024, quantity, false, false);
                                }
                }
		...
		if (slea.available() == 0) { //Cash Shop :)
			...
			try {
				...
				if (targetid != -1 && !chr.isAlive()) {
					...
					chr.changeMap(to, to.getPortal(0));
				} else if (targetid != -1 && (!chr.isGM() || chr.isTemp())) {//Thanks celino for saving me some time (:
				...
				if (portal != null) {
					...
					if(c.getPlayer().getMapId() != 130010110)
