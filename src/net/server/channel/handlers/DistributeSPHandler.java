...
public final class DistributeSPHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (skillid == 2111003) { // Poison Mist (can Crash maps)
                       c.getPlayer().dropMessage(6, "This skill has been disabled.");
			c.getSession().write(MaplePacketCreator.enableActions());
			return; 
		}
        
       else if (skillid == 4121006) { // Shadow Stars (can Crash maps)
                        c.getPlayer().dropMessage(6, "This skill has been disabled.");
			c.getSession().write(MaplePacketCreator.enableActions());
			return;
		} 
        
     else   if (skillid == 5001005) { // Dash (Pirate)
                        c.getPlayer().dropMessage(6, "This skill has been disabled.");
			c.getSession().write(MaplePacketCreator.enableActions());
			return;
		} 
        
       else if (skillid == 21001001) { // Combat Step
                        c.getPlayer().dropMessage(6, "This skill has been disabled.");
			c.getSession().write(MaplePacketCreator.enableActions());
			return;
		} 
        else if (skillid == 2311002) { // Combat Step
                        c.getPlayer().dropMessage(6, "This skill has been disabled.");
			c.getSession().write(MaplePacketCreator.enableActions());
			return;
		}
