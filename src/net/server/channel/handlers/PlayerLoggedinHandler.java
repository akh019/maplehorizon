...
//    public static AutoSave auto = new AutoSave();
...
//               if(c.isJailed(player.getName()))
                //     player.setMapId(450004250);
//                auto.Autosaveplayer(player);
                newcomer = true;
        ...
        player.announce(MaplePacketCreator.setNPCScriptable(2101, "Tutorial1"));
        player.setRates();
        player.completeAchievement(MapleAchievementType.JOIN_SERVER);
        ...
        if (newcomer) {
            ...
            if (player.isGM()) {
               // player.getClient().getWorldServer().getDiscord().getBot().discMsg(player.getName() + " has logged in!", "staff");
                Server.getInstance().broadcastGMMessage(MaplePacketCreator.earnTitleMessage("GM " + player.getName() + " has logged in"));
            }
        }
        /*if (c.isJailed(player.getName())) {
            try {
                player.changeMap(450004250);
            } catch (SQLException ex) {
