...
import server.minigames.solo.PandaJump;
...
public final class TakeDamageHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (damagefrom != -3 && damagefrom != -4) {
            ...
            if (c.getPlayer().getMap().bombkillOn() && monsteridfrom == 9300166) {
                c.getPlayer().setHp(0);
                c.getPlayer().updateSingleStat(MapleStat.HP, 0);
            }
            } else if (monsteridfrom == 9400551) {
				if (damagefrom != -2 && player.getGMLevel()) {
					player.setHpMp(0);
				}
            else if(c.getPlayer().isShp() && monsteridfrom == 9300166){
                c.getPlayer().setSecondaryHp(c.getPlayer().getSecondaryHp()-1);
                if(c.getPlayer().getSecondaryHp() <= 0){
                  c.getPlayer().setHp(0);
                  c.getPlayer().updateSingleStat(MapleStat.HP, 0);
                  c.getPlayer().setShpBol(false);
                }
                 c.getPlayer().announce(MaplePacketCreator.earnTitleMessage("Secondary Hp: " + c.getPlayer().getSecondaryHp() + " / " + c.getPlayer().getInitialSecondaryHp()));
            ...
            if (attacker != null) {
                ...
                if (map.getId() == 450005010) {
                    if (damage > 1) { //hit by a spider
                        if (!player.isGM() || player.isTemp()) {
                            player.setHp(0);
                        }
                        player.updateSingleStat(MapleStat.HP, 0);
                        player.message("You were caught by a spider! Please try to be more careful next time!");
                    }
                }
        ...
        if (damage == -1) {
            ...
            GameManager gm = c.getPlayer().getGameManager();
            if (gm != null) { // Games take priority!
                if(gm.getCurrentGame() instanceof PandaJump) {
                    if(monsteridfrom == 9300270) {
                        gm.getCurrentGame().endGame();
                    }
                }
            }
            if (c.getPlayer().getMap().mobkillOn()) {
                c.getPlayer().setHp(0);
                c.getPlayer().updateSingleStat(MapleStat.HP, 0);
            }
            else if(c.getPlayer().isShp() && monsteridfrom != 9300166){
