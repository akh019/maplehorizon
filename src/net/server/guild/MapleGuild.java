...
public class MapleGuild {
    ...
    public static boolean isGuildNameChangeAcceptable(String name) {
    ...
    public void changeName(String newName) {
    	...
    	writeToDB(false);
    	sendGuildInfo();
    	Server.getInstance().getWorld(world).reloadGuildSummary();
    }
    
    public void sendGuildInfo() {
        for (MapleGuildCharacter mgc : members) {
            for (Channel cs : Server.getInstance().getChannelsFromWorld(world)) {
            	MapleCharacter player = cs.getPlayerStorage().getCharacterById(mgc.getId());
                if (player != null) {
