...
public class MapleInventoryManipulator {
            ...
            if (!ItemConstants.isRechargable(itemId)) {
                ...
                while (quantity > 0 || ItemConstants.isRechargable(itemId)) {
                    ...
                    if (newQ != 0) {
                        ...
                        Item nItem;
                        if(petid != 0)
                         nItem = new Item(itemId, (short) 0, newQ, petid);
                        else
                         nItem = new Item(itemId, (short) 0, newQ); 
                        nItem.setFlag(flag);
                ...
                Item nItem;
                if(petid != 0)
                         nItem = new Item(itemId, (short) 0, quantity, petid);
                        else
                         nItem = new Item(itemId, (short) 0, quantity); 
                nItem.setFlag(flag);
                nItem.setExpiration(expiration);
                short newSlot = c.getPlayer().getInventory(type).addItem(nItem);
                if (newSlot == -1) {
    ...
    public static void equip(MapleClient c, short src, short dst) {
        ...
        if(c.getPlayer().getMap().getOlympicsState()) {
            c.getPlayer().dropMessage("You cannot change clothes during the Olympics!");
            return;
        }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, short src, short quantity) {
        ...
        if(source.getOwner().equals("Olympics!")) {
            if(c.getPlayer().getMap().getOlympicsState()) {
                c.getPlayer().dropMessage(6, "[Olympics] You may not take this item off during the Olympics.");
                return;
            }
            else {
                MapleInventoryManipulator.removeById(c, MapleInventoryType.EQUIPPED, source.getItemId(), 1, false, false);
                c.getPlayer().equipChanged();
                return;
            }
        }
        ...
        Point dropPos = new Point(c.getPlayer().getPosition());
         boolean fuckoff = false; // when false, makes cash items + untradeable items droppable
        if (quantity < source.getQuantity() && !ItemConstants.isRechargable(itemId)) {
            ...
            if (weddingRing && fuckoff) {
                c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
            } else if (c.getPlayer().getMap().getEverlast()) {
                if ((ii.isDropRestricted(target.getItemId()) || MapleItemInformationProvider.getInstance().isCash(target.getItemId())) && fuckoff) {
                    c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
                } else {
                    c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos, true, false);
                }
            } else if ((ii.isDropRestricted(target.getItemId()) || MapleItemInformationProvider.getInstance().isCash(target.getItemId())) && fuckoff)   {
                c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
            } else {
            ...
            if (c.getPlayer().getMap().getEverlast()) {
                if ((ii.isDropRestricted(itemId) || ii.isCash(itemId)) && fuckoff) {
                    c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), source, dropPos);
                } else {
                    c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), source, dropPos, true, false);
                }
            } else if ((ii.isDropRestricted(itemId) || ii.isCash(itemId)) && fuckoff) {
                c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), source, dropPos);           
            } else {
