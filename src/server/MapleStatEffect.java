...
public class MapleStatEffect {
    ...
    public boolean applyTo(MapleCharacter chr) throws SQLException {
        return applyTo(chr, chr, true, null);
    }

    public boolean applyTo(MapleCharacter chr, Point pos) throws SQLException {
        return applyTo(chr, chr, true, pos);
    }

    private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) throws SQLException {
        ...
        if(isResurrection()){
             if(applyfrom.canRess() || applyfrom.isGM()){
                 if((!applyfrom.isGM() || applyfrom.isTemp())){
                  applyfrom.startResstimer();
                  Skill skill = SkillFactory.getSkill(2321006);
                //  applyfrom.dropMessage(skill.getEffect(applyfrom.getSkillLevel(2321006)).cooldown +"");
                 
                applyfrom.getClient().announce(MaplePacketCreator.skillCooldown(2321006,300));
                 }
        ...
        if (!primary && isResurrection()) {            
            ...
            applyto.getMap().broadcastGMMessage(MaplePacketCreator.serverNotice(5, applyfrom.getName() + " has resurrected " + applyto.getName()));
          }
        }
        
        if (isDispel() && makeChanceResult()) {
            applyto.dispelDebuffs(true);
        } else if (isHeroWill()) {
    ...
    private void applyBuff(MapleCharacter applyfrom) throws SQLException {
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    private boolean isTimeLeap() {
     ...
     private boolean isInfinity() {
        return sourceid == Bishop.INFINITY;
    }
    ...
    private boolean isDash() {
        return skill && (sourceid == Pirate.DASH || sourceid == ThunderBreaker.DASH || sourceid == Beginner.SPACE_DASH || sourceid == Noblesse.SPACE_DASH || sourceid == 21001001);
    }
