...
public class MapleMapFactory {
    ...
    public MapleMap getMap(int mapid) throws SQLException {
                 ...
                 Connection con = DatabaseConnection.getConnection();
                  PreparedStatement ps = con.prepareStatement("SELECT * FROM spawns WHERE mid = ?");
                    ...
                    while (rs.next()) {
                        ...
                        AbstractLoadedMapleLife myLife = loadLifeforNPC(id, f, hide, fh, cy, rx0, rx1, x, y, type);
                        if (type.equals("n")) {
                   ...
                   System.out.print("Error getting pnpc: " + e);
                }
                
                try { 
                    try (PreparedStatement psa = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM playernpcs WHERE map = ?")) {
                        psa.setInt(1, omapid);
                        try (ResultSet rsa = psa.executeQuery()) {
                            while (rsa.next()) {
                                map.addMapObject(new PlayerNPCs(rsa));
                            }
                        }
                    }
                } catch (Exception e) {
    ...
    private AbstractLoadedMapleLife loadLifeforNPC(int id, int f, boolean hide, int fh, int cy, int rx0, int rx1, int x, int y, String type) {
    ...
    public MapleMap disposeMap(int mapId) throws SQLException {
