...
public class StringUtil {
	...
	public static String joinStringFrom(String arr[], int start) {
        ...
        public static boolean isSpam(String name){
            ...
            if(numbers > 4 && letters > 0)
                return true;
            
            int capitals = 0,normals = 0;
          for(int i = 0; i < name.length(); i++){
	...
	public static int getOptionalIntArg(String arr[], int position, int def) {
        if (arr.length > position) {
            try {
                return Integer.parseInt(arr[position]);
	...
	public static String getReadableMillis(long startMillis, long endMillis) {
		return getReadableMillis(startMillis, endMillis, true);
	}
	
	public static String getReadableMillis(long startMillis, long endMillis, boolean seconds) {
        StringBuilder sb = new StringBuilder();
        double elapsedSeconds = Math.abs((endMillis - startMillis) / 1000.0);
        ...
        if (elapsedDays > 0) {
            ...
            if (mins) {
                boolean secs = elapsedMins > 0 && seconds;
                if (!secs) {
                    sb.append("and ");
                }
                ...
                if (secs) {
                    ...
                    if (millis) {
                        ...
                        sb.append(" second").append(elapsedSecs > 1 ? "s" : "");
                    }
                }
            }
            ...
            if (mins) {
                boolean secs = elapsedSecs > 0 && seconds;
                if (!secs) {
                    sb.append("and ");
                }
                sb.append(elapsedMins);
            ...
            sb.append(" second").append(elapsedSeconds > 1 ? "s" : "");
        }
        return sb.toString();
    }
}